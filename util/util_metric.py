...
def evaluate(data_iter, net):
    ...
    label_real = []
    rep_list = []
    for embed_data, pos, y in data_iter:
        embed_data, pos, y = embed_data.cuda(), pos.cuda(), y.cuda()
        outputs, rep = net(embed_data, pos)  # output为最后分类的（15，2）维张量，rep为block1输出后的（15，128）维张量
    ...
    return performance, roc_data, prc_data, rep_list, label_real, label_pred, pred_prob
...
def caculate_metric(pred_prob, label_pred, label_real):
    ...
    performance = [ACC, Sensitivity, Specificity, AUC, MCC, BACC]
